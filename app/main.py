...
async def _process_message(message: dict) -> None:
    ...
    if approval:
        try:
            # Staged for confirmation, never sent straight off a typed line: with
            # an edit pending, anything typed here would otherwise go to a live
            # prospect — including a stray note typed hours later.
